        ...
        <div className="p-4 space-y-4">
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              ...
              <button onClick={() => fileInputRef.current?.click()} className="text-xs flex items-center text-blue-600 dark:text-blue-400">
            ...
            <textarea className="w-full h-40 bg-gray-50 dark:bg-gray-800 rounded-xl p-3 text-sm font-mono focus:ring-2 focus:ring-gray-900 outline-none resize-none" placeholder={t('pastePlaceholder')} value={inputText} onChange={(e) => setInputText(e.target.value)} />
          </div>
          {status && (
            <div className={`p-3 rounded-lg text-sm flex items-center ${status.type === 'success' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
              {status.message}
            </div>
          )}
          <button onClick={handleImportAction} disabled={!inputText.trim()} className="w-full py-3 bg-gray-900 text-white rounded-xl disabled:opacity-50">
